    ...
    for row in range(num):
        for col in range(num):
            if col == row:
                ...
                p1 = raw_list[row]
                p2 = raw_list[col]
                arr[row][col] = round(math.sqrt(math.pow((p1.x - p2.x), 2) + math.pow((p1.y - p2.y), 2)),2)  ### 求两点见距离，保留2位小数
    return num,arr
...
### 参数：最小路径的最后一个节点和邻域
def valSimulateAnnealSum(curnode, nextnodeList, t,arr):
...
if __name__ == '__main__':
    ...
    # print("最合适的路径为："+str(slist))
    print("路径节点个数："+str(len(slist)))
    print("最小花费为："+str(finalcost))
